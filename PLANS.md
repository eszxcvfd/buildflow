...
- **Follow-up (correlation, wire-all):** wire `X-Correlation-Id` into the remaining audit producers — logout, request/confirm reset, change-password, update-profile (lenient: invalid/absent → undefined, never 400), create-user + update-user (strict: invalid UUID → 400 with actionable VN message), and org workers/contractors create+update (strict, admin-guarded); dedup index `ux_audit_correlation_action` now covers every audit producer in `src/api` (no exceptions). Proof: typecheck/lint/jest green (428 passed + 11 DB-gated integration on ephemeral PostgreSQL); API.md §8.2 producer coverage rewritten to final state.
