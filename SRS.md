...
### **14.1.1. Quyết định đã chốt**

| Mã | Kết quả | Ghi chú |
| --- | --- | --- |
| Q-01 | Giữ riêng vai trò Quản lý dự án và Điều phối viên | Ma trận quyền §2.4 giữ nguyên. |
| Q-02 | Phân công trực tiếp có hiệu lực ngay, không bước xác nhận | Nhất quán tinh thần BR-02; Assignment ghi nguồn tạo (self-claim/direct). |
| Q-03 | Giới hạn việc đồng thời tính theo số lượng: các Work Order ở trạng thái Đã phân công + Đang thực hiện; mặc định 3, cấu hình được | Thuật ngữ Concurrent limit trong `CONTEXT.md`. |
| Q-04 | Không áp dụng cooldown sau khi worker hủy trong v1 | Nếu cần sau này, thêm eligibility policy, không đổi schema. |
| Q-05 | Hai chế độ: self-claim bị chặn hoàn toàn khi trùng lịch; direct assignment được cảnh báo và ghi đè kèm lý do | Chế độ ghi đè thuộc phạm vi direct assignment (slice sau). |

Chốt ngày 25/08/2026 qua phiên grilling rút gọn; xem `CONTEXT.md` và `docs/adr/`. Các quyết định còn lại (Q-06 → Q-12) chưa chốt, xử lý trước thiết kế/nghiệm thu tương ứng.
