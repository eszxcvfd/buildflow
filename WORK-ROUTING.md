...
Khung hiện có trong `src/` gồm `api/`, `web/` và `mobile/`; `infra/docker/compose.yaml` là target data-layer path và hiện chưa tồn tại. Các workspace chưa có source/config; mọi entry point bên dưới là target design cho giai đoạn scaffold, không phải bằng chứng runtime.
...
| PostgreSQL schema/migration/repository adapter | `src/api` + `infra` / Data | [`docs/architecture/DATA.md`](docs/architecture/DATA.md) + [`docs/architecture/API.md`](docs/architecture/API.md) | compose/health, migration và repository integration; không cho client truy cập trực tiếp |
| Redis key/TTL/cache/coordination policy | `src/api` + `infra` / Data | [`docs/architecture/DATA.md`](docs/architecture/DATA.md) + [`docs/architecture/API.md`](docs/architecture/API.md) | adapter, TTL/eviction, cache-miss và outage behavior |
| Docker Compose/image/volume/network/healthcheck | `infra/docker` / Data | [`docs/architecture/DATA.md`](docs/architecture/DATA.md) + [`docs/architecture/CONTENT.md`](docs/architecture/CONTENT.md) | `docker compose config`, service health và secret/volume audit |
...
9. PostgreSQL là system of record; Redis mặc định là cache/coordination không authoritative và không được chứa domain truth.
10. API sở hữu migration, repository/cache port và adapter; Docker Compose chỉ sở hữu process, network, volume và healthcheck của data services.
11. Web/mobile không kết nối trực tiếp tới PostgreSQL/Redis; mọi data access phải đi qua API contract.
...
- Thay đổi data layer: [`docs/architecture/DATA.md`](docs/architecture/DATA.md), `API.md`, `RUNTIME.md` và `CONTENT.md` nếu chạm package/volume.
...
- Nếu một thay đổi thuộc nhiều lane, lane Contract/Runtime/Data làm coordinator và ghi rõ các consumer phải đồng bộ.
- Data change route từ data producer/adapter trước; nếu đổi source of truth, Redis durability hoặc production topology thì phải cập nhật ADR.
...
- data target path, owner, healthcheck và persistence/cache policy được link nhất quán;
- khi có implementation: Compose config, PostgreSQL readiness/migration và Redis PING/TTL/outage proof.
