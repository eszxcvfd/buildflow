...
-- DBD-CWM-QC-002 v2.1 reconciled with CR-001 / PostgreSQL 18.x
-- Migration: 20260902000000_dbd_v2_1_baseline
--
-- This is a forward-only schema migration. It creates the 26 business tables
-- and 8 system-support tables described in docs/foundation/DATA-BASELINE.md.
-- Cross-table business rules (eligibility, dependency cycles, readiness gates,
-- Hold Point release and quality close) remain service/transaction rules.
...
CREATE TABLE public.work_order_blockers (
  ...
  responsible_party_type varchar(15) NOT NULL,
  ...
  CONSTRAINT work_order_blockers_party_type_ck CHECK (
    responsible_party_type IN ('USER', 'CREW', 'EXTERNAL')
  ),
  CONSTRAINT work_order_blockers_responsible_party_ck CHECK (
    (responsible_party_type = 'USER' AND responsible_user_id IS NOT NULL AND responsible_crew_id IS NULL)
    OR
    (responsible_party_type = 'CREW' AND responsible_user_id IS NULL AND responsible_crew_id IS NOT NULL)
    OR
    (
      responsible_party_type = 'EXTERNAL'
      AND responsible_user_id IS NULL
      AND responsible_crew_id IS NULL
      AND NULLIF(BTRIM(responsible_note), '') IS NOT NULL
    )
  ),
  ...
  CONSTRAINT work_order_blockers_resolution_ck CHECK (
    status <> 'RESOLVED'
    OR (
