...
| `GET /api/v1/job-board` (list, JOB-SRS-005, `#45`, module `job`) | OK (tất cả available, bypass chỉ debug-log — không audit row) | OK (PM/…) | OK (**member** — chỉ WO available của các project mình là ACTIVE member; membership rỗng → `200 []`, không `403`; không filter param scope để tamper; xem `ENDPOINTS.md` §20) | `401` |
