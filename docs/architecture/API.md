...
| `POST /api/v1/work-orders/:id/job-board/open` (JOB-SRS-004, `#44`, module `job`) | OK (bypass, audited; missing → `404`) | OK chỉ khi ACTIVE member MANAGER/COORDINATOR của project chứa WO (non-member → `403` kể cả id missing; WORKER member → `403`) | `403` | `401` |
| `POST /api/v1/work-orders/:id/job-board/close` (JOB-SRS-004, `#44`, module `job`) | OK (bypass, audited; missing → `404`) | OK chỉ khi ACTIVE member MANAGER/COORDINATOR của project chứa WO (non-member → `403` kể cả id missing; WORKER member → `403`) | `403` | `401` |
| `GET /api/v1/job-board` (list + filter, JOB-SRS-005 `#45` + JOB-SRS-006 `#46`, module `job`) | OK (tất cả available, bypass chỉ debug-log — không audit row) | OK (PM/…) | OK (**member** — chỉ WO available của các project mình là ACTIVE member; membership rỗng + không gửi `projectId` → `200 []`, không `403`; đã gửi `projectId` ngoài scope → `403` generic — BD12; filter `projectId` đơn + `areaId`/`workTypeId` lặp + `dateFrom`/`dateTo` ISO bắt buộc offset (planned overlap) + `skill=mine`; xem `ENDPOINTS.md` §20–§20.1) | `401` |
| `GET /api/v1/job-board/filter-options` (picker, JOB-SRS-006 `#46`, module `job`) | OK (toàn board, bypass chỉ debug-log — không audit row) | OK (PM/…) | OK (**member** — DISTINCT trên đúng availability+scope + `now` top-level (clock server ISO string); membership rỗng → `200` 4 mảng rỗng + `now`; xem `ENDPOINTS.md` §20.1 BD13) | `401` |
| `GET /api/v1/job-board/:id` (detail, JOB-SRS-007 `#47`, module `job`) | OK (bypass, audited; missing → `404`) | OK (member) | OK (**member** — mọi ACTIVE member, kể cả WORKER; non-member → `403` generic kể cả id không tồn tại; xem `ENDPOINTS.md` §20.2) | `401` |
...
- **Mở/đóng Job Board (JOB-SRS-004, `#44`, xem `ENDPOINTS.md` §19):** `POST /api/v1/work-orders/:id/job-board/open` (`{ jobBoardOpenFrom? (default now), jobBoardOpenUntil? (null = không hạn), expectedVersion?, reason? ≤500 }`) và `POST .../close` (`{ expectedVersion?, reason? }`) — luôn `200` (command precedent work-types status), write-scope của project chứa WO (request well-formed: non-member `403` kể cả id missing, ADMIN missing → `404`; request malformed → `400` trước scope-check), strict `X-Correlation-Id`; DTO whitelist-only, use-case single-validator (F005); open gate deterministic (normalize pure TRƯỚC scope → pre-check board replay/409 (không gửi `jobBoardOpenFrom` + board mở → replay intent `alreadyOpen` kể cả version stale — idempotency-wins F024; until-only update KHÔNG hỗ trợ, F023; explicit so FULL-MS, F016) → assignment `PENDING_ACCEPTANCE`/`ACTIVE` 409 → status 400 → readiness re-check tại thời điểm ghi, re-open lọc `INVALID_STATUS_FOR_PUBLISH` → guarded UPDATE flag+status+version+NOT EXISTS, race-path replay-check TRƯỚC classify — F015/F022) → `OPEN` + `version`+1, audit `JOB_BOARD_OPENED` + state_history chỉ khi status đổi; replay cùng window → `200 alreadyOpen` (pre-check không mở tx, race-path commit tx rỗng, không audit mới; `expectedVersion` lệch → `409 WORK_ORDER_CONFLICT` trừ khi replay thắng trước — F024); close → `OPEN`→`READY` (giữ from/until, không đụng assignments), đã đóng → `200 alreadyClosed`; `expectedVersion` lệch → `409 WORK_ORDER_CONFLICT`; `GET :id` trả thêm `jobBoard: { open, openFrom, openUntil, hasActiveAssignment, state }` server-derived (`ASSIGNED`/`EXPIRED`/`SCHEDULED`/`AVAILABLE`/`CLOSED`; list không enrich — defer `#45`); cờ `alreadyOpen`/`alreadyClosed` nằm top-level response (KHÔNG trong `jobBoard`); lỗi nêu nguyên nhân + cách xử lý (`JOB_BOARD_WINDOW_INVALID` + `fieldErrors {jobBoardOpenFrom/jobBoardOpenUntil}` — gồm ISO thiếu offset, `WORK_ORDER_NOT_PUBLISHABLE` + `unmet[]`, `WORK_ORDER_STATUS_NOT_OPENABLE` + `fieldErrors {status}`, `JOB_BOARD_ALREADY_OPEN`, `JOB_BOARD_HAS_ASSIGNEE`, `JOB_BOARD_VERSION_INVALID` + `fieldErrors {expectedVersion}`, `JOB_BOARD_REASON_TOO_LONG` + `fieldErrors {reason}`, `WORK_ORDER_STATUS_NOT_CLOSABLE` + `fieldErrors {status}` chỉ race `CANCELLED`); id sai UUID / correlation sai / field lạ → `400` stock Nest KHÔNG `code`/`fieldErrors`. Không migration mới (cột + CHECK + index từ 0001); `updateWithClient` không đụng job_board columns (PATCH không clobber — R3); không notification trong slice này (SRS không mandate, worker khám phá qua board `#45`).
