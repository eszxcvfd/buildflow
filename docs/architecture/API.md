...
PostgreSQL và Redis được chọn cho data baseline; chi tiết Docker/Compose, persistence, TTL/eviction, migration và production topology thuộc [`DATA.md`](DATA.md). ORM đã chốt Prisma ([`ADR-0003`](../adr/0003-orm-prisma-postgresql.md)); auth provider và production hosting chưa được chốt.
