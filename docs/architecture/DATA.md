...
| 0005 | `0005_prj_srs003_project_areas.sql` | Bảng `public.project_areas` (có từ baseline 0001 — KHÔNG bảng mới): `code` DROP NOT NULL (optional); FK đổi tên `fk_project_areas_project_id ... ON DELETE RESTRICT`; `CHECK project_areas_name_ck (btrim(name) <> '')`; partial unique `ux_project_areas_active_name (project_id, name) WHERE is_active` (**case-sensitive btree — đã thay bằng 0006**) | khu vực dự án một cấp PRJ-SRS-003 (`#34`): single-level enforced by schema (không có cột `parent_id`); tên active duy nhất mỗi project; xóa project còn areas bị RESTRICT; xem [`ENDPOINTS.md`](ENDPOINTS.md) §13 |
