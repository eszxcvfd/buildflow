...
Response template profile: `{ id, code, name, description, workTypeId, requiredTradeId, defaultDurationMinutes, defaultPriority, requiredSkills, checklistSnapshot, sourceChecklistTemplateId, status, version, usableForNewWorkOrder, createdAt, updatedAt, versionChanged?, alreadyInState? }` (`usableForNewWorkOrder` = đang ACTIVE; `versionChanged` chỉ ở PATCH; `alreadyInState` chỉ ở status endpoint).
