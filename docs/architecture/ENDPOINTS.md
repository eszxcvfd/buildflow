...
| GET | `/api/v1/projects/:projectId/areas/active` | JWT bắt buộc, **mọi ACTIVE member** (picker cho giao dịch mới — xem A7) | — (luôn chỉ active, sắp `name ASC`) | `200 { data[], total }` + header `Cache-Control: no-store` | `404` dự án; `403` ngoài scope |
| POST | `/api/v1/projects/:projectId/areas` | **ADMIN + PROJECT_MANAGER** (+ scope A4) | `{ code?, name }` | `201` area | `400`/`404`/`403`/`409` (xem rules) |
| PATCH | `/api/v1/projects/:projectId/areas/:areaId` | **ADMIN + PROJECT_MANAGER** (+ scope A4) | `{ name?, code?, isActive?, reason? }` | `200` area + `alreadyInactive` (+ `usage`/`warning` khi retire — xem A7) | `400`/`404`/`403`/`409` (xem rules) |

Response `ProjectAreaDto`: `{ id, projectId, code (null = không mã), name, isActive, createdAt, updatedAt, usage?, warning?, alreadyInactive? }` (ISO từ timestamptz; `isActive=false` = đã retire, flag giữ lịch sử; `usage`/`warning` chỉ khi retire — xem A7).
...
- **A6 — audit `PRJ_PROJECT_AREA_ADDED` / `PRJ_PROJECT_AREA_UPDATED` + `PRJ_AREA_STATUS_CHANGED` (PRJ-SRS-007, `#38`)**, `entityType` `PROJECT`, `entityId`=projectId, tx-embedded `logWithClient` (fail closed — thiếu adapter → `500`), before/afterData = area row + `projectCode` (add: before null; update: `reason` ở cột `audit_logs.reason` và trong `afterData` khi gửi); catch-all audit fail → `500` rollback. `reason` reuse từ PATCH DTO (đã có sẵn từ `#34` — không thêm field). Mọi transition `isActive` (cả hai chiều deactivate/reactivate) ghi thêm `PRJ_AREA_STATUS_CHANGED` cùng tx (actor/before/after/reason — mirror `PRJ_WORK_TYPE_STATUS_CHANGED` `#35`); retire đang bị WO mở tham chiếu gắn `_warning` trong afterData cả hai audit rows. Tx với `FOR UPDATE` trên project row (create) / area row (update). Idempotent: double-deactivate → `200 {alreadyInactive:true}`, không mutation, không audit (mirror `alreadyRemoved` `#30`/`#36`); PATCH không thay đổi hiệu lực → no-op không audit. **`reason` trên các đường no-op / deactivate-repeat bị bỏ qua cố ý** (idempotent repeat, không ghi audit row — nhất quán precedent L1 idempotency-wins: no-op kể cả khi thiếu reason).
- Hai shape `400` như các slice trước (ValidationPipe shape không `fieldErrors` cho body thiếu/sai format cơ bản; business-invalid có `fieldErrors`). Strict `X-Correlation-Id` trên writes (POST/PATCH sai UUID → `400`; GET list + picker miễn).

## 13.1. Vòng đời dữ liệu nền — PRJ-SRS-007 (#38) bounded decisions (areas)

Đóng gaps areas còn lại của PRJ-SRS-007 (trades/work-types/contractors/crews/workers đã đạt ở slice trước — xem § tương ứng). Closest precedent: work-types `#35` (usage/warning shape, status audit, picker `/active`).

- **A7 — picker + usage/warning (mirror work-types W5/W6):** `GET /projects/:projectId/areas/active` là endpoint picker ngữ nghĩa cho giao dịch mới (tạo Work Order) — chỉ active, sắp `name ASC`, scope như list (mọi ACTIVE member, kể cả WORKER); `?activeOnly=true` trên list giữ nguyên cho filter tương đương. Không có `GET` area detail riêng nên `usage` trả ngay trên `PATCH` retire: `usage: { workOrders }` (đếm qua `countOpenWorkOrders`, pre-tx fail-fast — count fail → `500`, không transition thiếu cảnh báo) + `warning: 'Khu vực đang được tham chiếu bởi Work Order đang hiệu lực'` khi `usage > 0` (KHÔNG chặn retire — picker `/active` ẩn, WO cũ vẫn đọc). Reactivate/pure rename không đếm, không warning. Deactivate-race (revoke membership giữa pre-check và lock trong tx) → `403` rollback, không partial-write (re-check `assertMemberScopeTxCheck` sau `FOR UPDATE` — unit spec `#38`).
- **A8 — skills không phải catalog độc lập (ghi rõ theo yêu cầu slice):** kỹ năng là thuộc tính `skill_level` 1-5 của `resource_trades` (`CHECK resource_trades_skill_level_ck`, migration 0001) — KHÔNG có bảng `skills`, KHÔNG lifecycle riêng, KHÔNG endpoint riêng. Ngừng hoạt động của một kỹ năng = deactivate row `resource_trades` (`is_active=false`); ngừng cả ngành = deactivate catalog `trades` (đã có `ORG_TRADE_STATUS_CHANGED`). Worker search lọc đúng phạm vi hiệu lực: filter `tradeId`/`skillLevel` chỉ khớp row `resource_trades.is_active = true` (`pg-worker.repository.ts` `EXISTS ... rt.is_active = true`); trade catalog active enforce lúc gán (create/update worker → `400` khi trade không tồn tại/inactive). Eligibility `TRADE_SKILL_MATCH` đọc cùng nguồn (row hiệu lực + `skill_level` so ngưỡng).
- **A9 — rename policy (trades/work-types/areas, ghi rõ theo yêu cầu slice):** rename là đổi label tại chỗ, giữ nguyên id/FK (`area_id`/`work_type_id`/`trade_id` history không đụng) nên label hiển thị mới lan tỏa mọi nơi đang join, NHƯNG kết quả nghiệp vụ đã lưu không đổi: kết luận/nghiệm thu dùng FK + snapshot version tại thời điểm áp dụng (work-types `config_version` snapshot cho QUA-SRS-002; WO lưu version áp dụng khi JOB landing — xem W6). Diễn giải SRS 'hạng mục đã dùng không bị xóa làm mất lịch sử' (PRJ-SRS-003) / 'dữ liệu cũ vẫn hiển thị đúng' (PRJ-SRS-004): 'không sửa historical' = kết quả nghiệp vụ đã chốt, không phải label hiển thị hiện hành.
- **A10 — DB note work_orders FKs (quyết định: KHÔNG migration 0008):** `work_orders.area_id`/`work_type_id`/`required_trade_id` là inline `REFERENCES` (migration 0001, default `NO ACTION` — DB đã chặn xóa parent còn child tham chiếu, tương đương RESTRICT ở statement-level; không có `ON DELETE CASCADE/SET NULL` nên không mất liên kết âm thầm). API không expose DELETE/hard-delete cho areas/work-types/trades nên đường xóa parent không tồn tại ở application layer — đủ, không cần constraint tường minh thêm. Verify: không có orphan có thể phát sinh (không có đường xóa parent; JOB chưa tồn tại nên `work_orders` rỗng với các FK này). Nếu sau này cần tường minh hóa, làm migration `ADD CONSTRAINT ... ON DELETE RESTRICT` đặt tên (an toàn vì inline REFERENCES đã đảm bảo không orphan) — defer, không làm trong slice này.
...
- **S2 — enforce tại use-case level:** guard chạy TRƯỚC mọi 404/validation nghiệp vụ (anti-leak: non-ADMIN luôn 403 generic `Không có quyền truy cập dự án này` bất kể project tồn tại hay không; ADMIN check `exists()` trước → project missing vẫn 404). Areas create/update/list đã reorder scope-first theo review P1-1 (trước đây check 404 project trước scope). Controller prj bỏ `requireRoles` trên PATCH project/status + members routes (giữ cho `POST /projects` create và areas writes).
...
- **S4 — anti-leak:** 403 không phân biệt tồn tại/không (giữ như detail path). Mọi handler có project id đều qua guard (bảng trên + areas A4 + iam reads); areas scope-first: non-ADMIN luôn 403 kể cả project không tồn tại, ADMIN giữ 404.
- **Audit decisions:**
...
## 16. Work order templates — PRJ-SRS-008 (#39) bounded decisions

Quản lý mẫu công việc: mô tả + thời lượng dự kiến + kỹ năng + checklist snapshot + status/version để dùng khi tạo Work Order (SRS.md PRJ-SRS-008; actor Điều phối viên). API slice; UI Web thuộc web slice riêng (issue #39 giữ nguyên scope full-stack — slice này chỉ đóng phần API). Closest precedent: work-types `#35` (entity/repo/audit/optimistic-locking/picker shape) + areas `#34` (409 code shape, idempotency, audit tx-embedded).

| Method | Path | Auth | Body | Response | Lỗi |
| --- | --- | --- | --- | --- | --- |
| POST | `/api/v1/work-order-templates` | **ADMIN + PROJECT_MANAGER** | `{ code, name, description?, workTypeId?, requiredTradeId?, requiredSkills?, checklistSnapshot?, sourceChecklistTemplateId?, defaultDurationMinutes?, defaultPriority? }` (khởi tạo `DRAFT`, `version` 1) | `201` template profile | `400`; `409` trùng `code` |
| GET | `/api/v1/work-order-templates` | **ADMIN + PROJECT_MANAGER** | query `status` (`DRAFT`/`ACTIVE`/`INACTIVE`/`ALL`, thiếu = ALL), `workTypeId` (uuid), `search` (ILIKE code/name), `limit` (1-100, default 20), `offset` (≥0) — sort cố định `ORDER BY name` | `200 { data[], total, limit, offset }` + header `Cache-Control: no-store` | `400` query sai (`{ statusCode, message, fieldErrors }`) |
| GET | `/api/v1/work-order-templates/active` | **ADMIN + PROJECT_MANAGER** | — (picker cho JOB: chỉ ACTIVE, `ORDER BY name`) | `200 { data[], total }` + header `Cache-Control: no-store` | `401`/`403` |
| GET | `/api/v1/work-order-templates/:id` | **ADMIN + PROJECT_MANAGER** | — (mọi status đều đọc được — phục vụ prefill + lịch sử) | `200` template profile + header `Cache-Control: no-store` | `400` id sai; `404` |
| PATCH | `/api/v1/work-order-templates/:id` | **ADMIN + PROJECT_MANAGER** | `{ code?, name?, description?, workTypeId?, requiredTradeId?, requiredSkills?, checklistSnapshot?, sourceChecklistTemplateId?, defaultDurationMinutes?, defaultPriority?, expectedVersion?, reason? }` | `200` template profile + `versionChanged` | `400`; `404`; `409` trùng code / conflict version |
| POST | `/api/v1/work-order-templates/:id/status` | **ADMIN + PROJECT_MANAGER** | `{ action: 'ACTIVATE' \| 'DEACTIVATE', reason? }` | `200` template profile + `alreadyInState` | `400` action sai / transition sai / publish rỗng; `404` |

Response template profile: `{ id, code, name, description, workTypeId, workType, requiredTradeId, defaultDurationMinutes, defaultPriority, requiredSkills, checklistSnapshot, sourceChecklistTemplateId, status, version, usableForNewWorkOrder, createdAt, updatedAt, versionChanged?, alreadyInState? }` (`usableForNewWorkOrder` = đang ACTIVE; `versionChanged` chỉ ở PATCH; `alreadyInState` chỉ ở status endpoint). `workType` = `{ id, code, name } | null` do API join sẵn từ `work_types` (kể cả loại đã ngừng — UI dùng thẳng, không lookup `/work-types/active`; `null` khi `workTypeId` NULL hoặc ref thiếu).

- **T1 — schema (migration 0008, bảng mới):** `work_order_templates` (`id/code/name/description/work_type_id→work_types/required_trade_id→trades/default_duration_minutes/default_priority/required_skills jsonb/checklist_snapshot jsonb/source_checklist_template_id→checklist_templates/status DEFAULT 'DRAFT'/version DEFAULT 1/timestamps`) + `CHECK` status (`DRAFT|ACTIVE|INACTIVE`), version ≥ 1, duration > 0, priority enum + `ux_work_order_templates_code` + `ix_work_order_templates_type_status`. `required_skills`/`checklist_snapshot` shape validate ở app layer (policy + entity + repo guard — không DB CHECK để thêm skill/checklist kind không cần đổi schema, mirror `required_fields` #35). **KHÔNG FK tới `work_orders`** (snapshot-copy — sửa mẫu không hồi tố WO đã tạo; `work_orders` cũng không có cột `template_id`).
- **T2 — uniqueness + constraint-order:** pre-check `findByCode` case-insensitive (`lower(code)`) + race guard trong tx map đúng tên `ux_work_order_templates_code` → `409 {code:'WORK_ORDER_TEMPLATE_CODE_DUPLICATE'}`; **constraint cụ thể trước, bare `23505` rethrow** (precedent areas A3/work-types W2). **Residual đã chấp nhận:** `ux` btree case-sensitive nên case-race đồng thời như work-types W2; sequential behavior CI-unique.
- **T3 — reference validation:** `workTypeId` khi gửi phải tồn tại VÀ ACTIVE (`findActiveWorkTypeById` đọc trực tiếp `public.work_types`) — ngược lại `400 fieldErrors {workTypeId}`; `requiredTradeId` tương tự qua `findActiveTradeById` (`{requiredTradeId}`); `requiredSkills[].code` là trade code — từng code phải tồn tại VÀ ACTIVE (`findActiveTradeByCode`, `{requiredSkills}`); `sourceChecklistTemplateId` khi gửi phải tồn tại (`findChecklistTemplateSnapshot`, `{sourceChecklistTemplateId}`). UUID sai format → `400` fieldErrors field tương ứng.
- **T4 — snapshot-copy:** khi tạo, nếu gửi `sourceChecklistTemplateId` mà omit `checklistSnapshot` thì copy items của source (`title/answer_type/is_required/is_blocking/requires_photo/sequence_no` → shape snapshot) vào `checklist_snapshot`; snapshot gửi tường minh luôn thắng. `sourceChecklistTemplateId` là provenance, không live link — đổi provenance không bump version. Sửa mẫu không đụng WO đã tạo (không có FK, không backfill).
- **T5 — optimistic locking (pre-check + SQL guard, mirror W4):** `PATCH` nhận `expectedVersion` optional; mismatch pre-check → `409 {code:'WORK_ORDER_TEMPLATE_CONFIG_CONFLICT', fieldErrors:{expectedVersion}}`. Không gửi = last-write-wins. Guard thật ở `PgWorkOrderTemplateRepository.save` (`UPDATE ... WHERE id=$ AND version=$expected`; rowcount 0 → 409 cùng code). Nội dung nghiệp vụ đổi (code/name/description/workType/trade/duration/priority/skills/checklist) → `version` +1 (`versionChanged: true`); PATCH no-op (kể cả chỉ đổi provenance mà provenance cũng giữ nguyên) → không audit.
- **T6 — lifecycle DRAFT→ACTIVE→INACTIVE (không hard delete):** `ACTIVATE` = DRAFT→ACTIVE (publish) hoặc INACTIVE→ACTIVE (tái phát hành); `DEACTIVATE` = ACTIVE→INACTIVE. DEACTIVATE từ DRAFT → `400 fieldErrors {action}` (nháp chưa publish giữ DRAFT). Publish guard (issue exception "thiếu skill/checklist reference"): ACTIVATE yêu cầu `requiredSkills` hoặc `checklistSnapshot` non-empty — ngược lại `400 fieldErrors {action}` nêu nguyên nhân + cách xử lý. Idempotent repeat → `200 {alreadyInState: true}`, không mutation, không audit. `GET /:id` đọc được mọi status; picker `/active` chỉ ACTIVE.
- **T7 — KHÔNG endpoint `/apply` (decision ghi rõ theo yêu cầu slice):** JOB module chưa tồn tại; prefill ở JOB-SRS-001 sẽ dùng `GET /active` + `GET /:id` rồi copy giá trị (snapshot, cho phép chỉnh trước khi lưu — đúng SRS "người dùng có thể chỉnh dữ liệu sau khi áp dụng mẫu"). Không mutate template qua prefill; không audit ở read path.
- **T8 — audit `PRJ_WO_TEMPLATE_CREATED` / `PRJ_WO_TEMPLATE_UPDATED` / `PRJ_WO_TEMPLATE_STATUS_CHANGED`**, `entityType` `WORK_ORDER_TEMPLATE`, `entityId`=templateId, tx-embedded `logWithClient` (fail closed → `500` rollback); update/status ghi `reason` ở cột `audit_logs.reason`. Strict `X-Correlation-Id` trên writes (POST/PATCH/POST-status sai UUID → `400`; GET miễn). Roles = ADMIN + PROJECT_MANAGER global catalog (mirror work-types — KHÔNG project-scope, xem §15 enforcement table).
- **T9 — `workType` enrichment trong profile (mirror crews `leaderName`):** mọi response template (list + `/active` + detail + create/update/status) kèm `workType { id, code, name } | null` — batch 1 query `findWorkTypeRefs` (`WHERE id = ANY($1::uuid[])`), không lọc `is_active` (mẫu tham chiếu loại đã ngừng như `WT-BE-TONG-TC` vẫn hiện tên). Web dùng thẳng `workType`, fallback id rút gọn chỉ khi `workTypeId` NULL → `—` hoặc ref thiếu. Lookup `/work-types/active` phía web giữ lại cho filter dropdown + picker trong form (chọn loại cho mẫu mới — chỉ loại ACTIVE), không dùng để resolve tên hiển thị.
- **Test contract:** `src/api/test/work-order-templates.e2e.spec.ts` (anon 401 / WORKER 403 / happy DRAFT→ACTIVE→edit→409 conflict→DEACTIVATE→picker / 409 code dup CI / 400 trade + work-type inactive + skill ghost + publish rỗng / X-Correlation-Id sai) + unit từng use-case + policy + mapper + controller spec.

> Lưu ý đánh số migration: §13.1 A10 từng ghi "không migration 0008" cho `work_orders` FKs — số hiệu `0008` nay dùng cho bảng mới của slice này; quyết định A10 về FKs (không cần constraint tường minh thêm) giữ nguyên.

## 17. Work orders — JOB-SRS-001 (#41) bounded decisions

Tạo Work Order nháp: điều phối viên tạo nháp gắn với dự án + khu vực (optional) + loại công việc + mô tả + ưu tiên + schedule + skill yêu cầu (SRS.md JOB-SRS-001). API slice trong module mới `job` (mirror cấu trúc slice work-types `#35`); UI Web thuộc slice riêng. Closest precedent: work-types `#35` (entity/repo/audit/code-dup) + areas `#34` (409 shape, idempotency) + project-access `#37` (scope-first P1-1).

> Đánh số section: `## 16` đã thuộc PRJ-SRS-008 (`#39`, WIP cùng thời điểm) nên slice này dùng `## 17` để không trùng heading.

| Method | Path | Auth | Body | Response | Lỗi |
| --- | --- | --- | --- | --- | --- |
| POST | `/api/v1/work-orders` | JWT + project write-scope (ADMIN bypass hoặc ACTIVE member MANAGER/COORDINATOR) | `{ projectId, workTypeId, title, areaId?, requiredTradeId?, code?, description?, instructions?, priority? (default NORMAL), plannedStartAt?, plannedEndAt? (ISO), plannedHeadcount? (1-99), requestKey? (uuid) }` (luôn `DRAFT`, `version` 1, `created_by` = actor) | `201` WO summary (`200` + `idempotentReplay: true` khi `requestKey` trùng) | `400` (gồm `WORK_ORDER_PROJECT_NOT_ACTIVE` khi project ≠ ACTIVE + `fieldErrors.projectId`); `403` ngoài scope (non-member kể cả project missing); `404` ADMIN + project missing; `409` trùng `code` |
| GET | `/api/v1/work-orders/:id` | JWT + project read-scope (ADMIN bypass hoặc bất kỳ ACTIVE member nào của project chứa WO — kể cả WORKER) | — | `200` WO summary + header `Cache-Control: no-store` | `400` id sai; `403` non-member (kể cả id không tồn tại); `404` ADMIN + id missing |
| GET | `/api/v1/work-orders/:id/publish-check` | JWT + project read-scope như `GET :id` (JOB-SRS-002 `#42`) | — | `200 { workOrderId, status, ready, unmet: [{code, field, message}], checkedAt }` + header `Cache-Control: no-store` | `400` id sai; `401` anon; `403` non-member (kể cả id không tồn tại); `404` ADMIN + id missing |

Response WO summary: `{ id, code, projectId, areaId, workTypeId, workTypeName?, requiredTradeId, title, description, instructions, priority, status, plannedStartAt, plannedEndAt, plannedHeadcount, createdBy, createdAt, updatedAt, version, idempotentReplay? }` (`requestKey` cố ý OMIT — chỉ nằm trong audit `afterData` phục vụ trace replay). `status` = full DB enum 8 giá trị (`work_orders_status_ck`, G1 rehydrate fix): `DRAFT|READY|OPEN|ASSIGNED|IN_PROGRESS|WORK_DONE|CLOSED|CANCELLED` — tạo mới luôn `DRAFT` (`createDraft` invariant), GET chấp nhận tất cả (`fromPersistence`).

- **J1 — scope-first create (lesson P1-1 `#37`):** actor server-derived từ JWT (không tin body); `ProjectScopeService.assertProjectWriteScope(projectId, actor)` chạy TRƯỚC mọi existence check (work-type/area/trade/code/requestKey) — non-ADMIN luôn `403` generic kể cả project không tồn tại (không leak `404`); ADMIN giữ `404` qua scope exists-check. Write = ADMIN bypass (audited) hoặc member MANAGER/COORDINATOR; membership là source of truth (PM global không member → `403`). **Gate project-ACTIVE (G3, #41) chạy SAU scope** (không leak existence — 1 query read trực tiếp `public.projects`, không migration): project missing → `404` (chỉ ADMIN tới được nhánh này; non-member đã `403` ở scope); project `≠ ACTIVE` (`DRAFT`/`PAUSED`/`COMPLETED`/`CLOSED`) → `400 { code: 'WORK_ORDER_PROJECT_NOT_ACTIVE', fieldErrors: { projectId } }`, không WO mới. **Phân biệt với advisory `#42` J6:** `WORK_ORDER_PROJECT_NOT_ACTIVE` = create gate chặn ghi; `PROJECT_NOT_ACTIVE` = unmet entry read-only trong `publish-check` (không chặn đọc).
- **J2 — draft minimal validation + active-reference checks:** `title` 1-200 bắt buộc; `workTypeId` bắt buộc + phải tồn tại VÀ ACTIVE → else `400 fieldErrors {workTypeId}` (`Loại công việc không tồn tại hoặc đã ngừng hoạt động`); `areaId` optional — khi gửi phải cùng project VÀ ACTIVE → `400 fieldErrors {areaId}`; `requiredTradeId` optional — khi gửi phải là trade ACTIVE → `400 fieldErrors {requiredTradeId}`; `priority` enum `LOW|NORMAL|HIGH|URGENT` default `NORMAL`; `planned_start_at`/`planned_end_at` optional ISO (cả hai → `start < end`, else `400 fieldErrors {plannedEndAt}`); `description`/`instructions` ≤ 5000; `plannedHeadcount` null hoặc 1-99. Thiếu dữ liệu tùy chọn (schedule/area/trade) → vẫn `DRAFT`, không chặn tạo.
- **J3 — idempotent `request_key` replay:** `requestKey` optional uuid; trùng `request_key` hiện có (pre-check) → `200` row HIỆN CÓ + `idempotentReplay: true`, KHÔNG insert mới, KHÔNG audit row mới (mirror `alreadyInState` semantics). Partial unique index `ux_work_orders_request_key (request_key) WHERE request_key IS NOT NULL` (migration 0009) — NULL không bao giờ match. **Race-replay (G2):** hai request đồng thời cùng key đều qua pre-check → insert thứ hai chạm `23505`/`ux_work_orders_request_key` trong tx (tx đã rollback) → outer-catch re-fetch `findByRequestKey` → `200` row thắng + `idempotentReplay: true`, không audit mới (mirror code-409 handling; row không còn → rethrow lỗi gốc).
- **J4 — code gen/dup 409:** `code` client-supplied optional theo `^[A-Za-z0-9][A-Za-z0-9-]{2,49}$`; trùng (pre-check CI `lower(code)` + race guard `ux_work_orders_code` trong tx, constraint-cụ-thể-trước, bare `23505` rethrow — mirror work-types W2) → `409 {code:'WORK_ORDER_CODE_DUPLICATE'}`; absent → server sinh `WO-` + base36 timestamp + 4 ký tự ngẫu nhiên (retry sinh lại khi chạm pre-check).
- **J5 — audit + không gán/job-board trong slice này:** audit `JOB_WORK_ORDER_CREATED` (`entityType` `WORK_ORDER`, `entityId`=woId, tx-embedded `logWithClient`, `beforeData` null / `afterData` public fields — không secret); audit fail → `500` rollback. Strict `X-Correlation-Id` trên POST (sai UUID → `400`; GET miễn). Nháp không bao giờ được gán, không lên job board (không endpoint nào trong slice — defer `#42`/`#44`).
- **Read scope (GET :id):** ADMIN bypass (audited, missing → `404`) hoặc bất kỳ ACTIVE member nào của project chứa WO (WORKER đọc được nháp của project mình); non-member luôn `403` generic — kể cả id không tồn tại (không phân biệt `404`/`403`, không leak).
- **Test contract:** `src/api/test/work-orders.e2e.spec.ts` (anon 401 / outsider 403 kể cả project missing / PM happy create→WORKER get / dup code 409 CI / inactive type 400 + area khác project 400 + trade inactive 400 + planned range 400 / replay idempotent 200 + không audit mới / bad correlation 400 / ADMIN missing 404) + unit entity/policy/mapper/use-case (scope-first ordering, 23505 race, replay không tx, audit reject → 500) + controller spec.
- **J6 — publish-check advisory JOB-SRS-002 (`#42`, gate tái dùng bởi `#44`/`#47`):** `GET /:id/publish-check` là advisory read-only — KHÔNG mutate, KHÔNG audit nghiệp vụ (check-only: GET không audit trong repo — scope service vẫn audit `ADMIN bypass`/`DENIED` theo behavior chung như `GET :id`), `no-store`, scope y hệt `GET :id`. Read qua read-port riêng (`JOB_PUBLISH_CHECK_READ_PORT`, 1 query JOIN `work_orders`+`projects`+`work_types`+`project_areas`+`trades`×2, không N+1, không migration mới). Readiness policy thuần domain, `unmet` deterministic theo catalog: `PROJECT_NOT_ACTIVE` (chỉ project `ACTIVE`) → `WORK_TYPE_MISSING` → `WORK_TYPE_INACTIVE` → `AREA_INVALID` (khác project/inactive; null=pass) → `MISSING_SCHEDULE` (per-field `plannedStartAt`/`plannedEndAt`) → `INVALID_SCHEDULE_RANGE` (strict `end > start`) → `MISSING_REQUIRED_SKILL` (`work-type.required_trade_id` có nhưng WO thiếu/khác/trade inactive) → `MISSING_REQUIRED_FIELD` (per-key từ `work_types.required_fields`) → `INVALID_STATUS_FOR_PUBLISH` (chỉ `DRAFT`/`READY`) → `ALREADY_ON_JOB_BOARD` (`job_board_open`). Mỗi unmet `{code, field, message}` tiếng Việt cụ thể; `ready = unmet` rỗng. `MISSING_REQUIRED_FIELD`: `work_orders` không có cột custom-fields nên key resolve về cột WO (title/code/description/instructions/priority/area/schedule/headcount/trade/project — so khớp case-insensitive, bỏ `_`/`-`); entry shape verify mirror work-types policy (không import cross-feature prj — sai shape bỏ qua, `required:false` miễn); key lạ không ánh xạ được cột → fail-closed `MISSING_REQUIRED_FIELD`. Publish/assign command ở `#44`/`#47` phải re-check server-side tại thời điểm ghi, không tin kết quả check cũ.

## 18. Attachments — PRJ-SRS-009 (#40) bounded decisions

Tài liệu đính kèm cơ bản gắn với dự án hoặc Work Order: tải lên, xem metadata, tải xuống, ngừng sử dụng (SRS.md PRJ-SRS-009; actor người có quyền trong dự án). API slice trong prj module (owner slice); WO là extension cùng bảng + cùng scope service (resolve `project_id` từ WO rồi delegate cùng use case — job module không đổi). Closest precedent: create-work-order `#41` (idempotency `requestKey` + audit tx-embedded) + project-access `#37` (scope-first P1-1, `ProjectScopeService` dùng chung).

| Method | Path | Auth | Body | Response | Lỗi |
| --- | --- | --- | --- | --- | --- |
| POST | `/api/v1/projects/:id/attachments` | JWT + project write-scope (ADMIN bypass hoặc ACTIVE member MANAGER/COORDINATOR) | multipart `file` (bắt buộc) + `caption?` (≤500) + `requestKey?` (uuid) (`attachmentType` server-fixed `DOCUMENT`) | `201` attachment profile (`200` + `idempotentReplay: true` khi `requestKey` trùng) | `400` file thiếu/quá hạn/sai loại/caption dài; `403` ngoài scope (kể cả project missing) |
| GET | `/api/v1/projects/:id/attachments` | JWT + project member-scope (ADMIN bypass hoặc bất kỳ ACTIVE member nào) | — (metadata cả active + inactive) | `200 { data[], total }` + header `Cache-Control: no-store` | `403` non-member |
| GET | `/api/v1/projects/:id/attachments/:attId/content` | JWT + project member-scope | — (att phải thuộc project trên path, else `404`) | `200` bytes + `Content-Type` gốc + `Content-Disposition: attachment` + `Cache-Control: no-store` | `403` non-member; `404` att missing/khác project |
| PATCH | `/api/v1/projects/:id/attachments/:attId/retire` | JWT + project write-scope | `{ reason? }` (≤500) | `200` attachment profile (`alreadyInactive: true` khi đã retire — không mutation, không audit) | `400` reason dài; `403` ngoài scope; `404` att missing/khác project |
| POST | `/api/v1/work-orders/:id/attachments` | JWT + project write-scope của project chứa WO | multipart mirror project (`owner_type=WORK_ORDER`) | `201` (`200` + `idempotentReplay` khi replay) | `400`; `403` non-member (kể cả WO missing); `404` ADMIN + WO missing |
| GET | `/api/v1/work-orders/:id/attachments` | JWT + project member-scope của project chứa WO | — | `200 { data[], total }` + `no-store` | `403` non-member; `404` ADMIN + WO missing |
| GET | `/api/v1/work-orders/:id/attachments/:attId/content` | JWT + project member-scope của project chứa WO | — (att phải thuộc WO trên path) | `200` stream mirror project | `403`; `404` |
| PATCH | `/api/v1/work-orders/:id/attachments/:attId/retire` | JWT + project write-scope của project chứa WO | `{ reason? }` | `200` mirror project | `400`; `403`; `404` |

Response attachment profile: `{ id, projectId, workOrderId, ownerType, fileName, mimeType, sizeBytes, caption, isActive, deactivatedAt, deactivatedBy, deactivateReason, createdAt, idempotentReplay?, alreadyInactive? }` (`storageKey`/path vật lý và `requestKey` cố ý OMIT — `requestKey` chỉ nằm trong audit `afterData` phục vụ trace replay).

- **AT1 — schema (migration 0010, chỉ ADD cột):** `is_active` boolean NOT NULL DEFAULT true + `deactivated_at` timestamptz NULL + `deactivated_by` uuid → `users` NULL + `deactivate_reason` varchar(500) NULL + `CHECK attachments_revocation_ck (is_active OR deactivated_at IS NOT NULL)` (mirror revocation style `project_members_revocation_ck`); `request_key` uuid NULL + partial unique `ux_attachments_request_key (request_key) WHERE request_key IS NOT NULL` (mirror 0009 — NULL không match). `IF NOT EXISTS` cả cột lẫn index.
- **AT2 — storage local disk:** layout `uploads/{projectId}/{uuid}-{safeName}` (`storageKey` relative, không lộ absolute path); adapter `LocalAttachmentStorageService` (`AttachmentStoragePort`: `save`/`read`/`remove`); base dir từ `UPLOADS_DIR` (compose mount named volume `uploads:/app/uploads` → `UPLOADS_DIR=/app/uploads`; local dev default `./uploads`; test truyền tmp dir tường minh). Key sinh từ uuid server-side + tên đã sanitize; resolve-check chặn traversal thoát baseDir tầng adapter lần nữa. Retire KHÔNG xóa file vật lý — history/download giữ.
- **AT3 — upload order (U1-U5):** normalize thuần (caption/requestKey, không I/O) → `assertProjectWriteScope` TRƯỚC mọi existence check (non-member luôn `403` kể cả project/WO missing; ADMIN giữ `404`) → replay `requestKey` TRƯỚC khi chạm disk (không file mới, không audit) → validate server-side toàn bộ trước khi ghi: size ≤10MB, mime allowlist (`image/jpeg,image/png,image/webp,application/pdf` — "loại file được phê duyệt"), magic-byte sniff (không tin header — EXE đổi tên `.pdf` bị reject; mismatch magic/khai báo bị reject), sanitize tên (path traversal → basename) → ghi file (ngoài tx) → DB tx (insert + audit `PRJ_ATTACHMENT_UPLOADED` tx-embedded). Multer dùng memory storage mặc định (buffer, chưa chạm disk) — KHÔNG diskStorage; giới hạn size enforce ở use case → `400` fieldErrors `{file}` deterministic (không phụ thuộc multer error mapping). Không giới hạn sẵn ở reverse-proxy là residual đã ghi (xem AT7).
- **AT4 — orphan cleanup + audit fail:** mọi lỗi trong DB tx (kể cả audit fail) → xóa file vừa ghi rồi mới trả lỗi; audit fail → `500` rollback (fail closed). Race đồng thời trùng `requestKey` (23505 `ux_attachments_request_key`) → xóa file vừa ghi, replay row thắng (`200` + `idempotentReplay: true`).
- **AT5 — permission map:** upload/retire = `assertProjectWriteScope` (ADMIN bypass audited hoặc ACTIVE member MANAGER/COORDINATOR); list/download = `assertProjectMemberScope` (ADMIN bypass hoặc bất kỳ ACTIVE member nào — WORKER đọc/tải được). Non-member luôn `403` generic (không leak `404`). WO extension: non-ADMIN + WO missing → `403`; ADMIN + WO missing → `404` (mirror `GetWorkOrderUseCase`).
- **AT6 — audit:** `PRJ_ATTACHMENT_UPLOADED` / `PRJ_ATTACHMENT_RETIRED` (`entityType` `ATTACHMENT`, `entityId`=attId, tx-embedded `logWithClient`; retire ghi `beforeData`/`afterData` public metadata). Replay và `alreadyInactive` KHÔNG ghi audit mới (mirror `alreadyInState` semantics). Strict `X-Correlation-Id` trên writes (POST/PATCH-retire sai UUID → `400`; GET miễn).
- **AT7 — residual đã chấp nhận:** (a) không giới hạn kích thước ở multer/reverse-proxy trong slice này — file >10MB vẫn vào RAM trước khi bị reject `400` (DoS note: production cần `client_max_body_size` + disk-storage streaming, pending owner); (b) không cây thư mục/versioning/chia sẻ công khai (đúng SRS "không bao gồm"); (c) `attachmentType` server-fixed `DOCUMENT` — các evidence type khác (PROGRESS/CHECKLIST/...) thuộc slice JOB tương ứng, dùng chung bảng; (d) container đang chạy dùng image cũ — volume/env có hiệu lực, code mới cần rebuild image (`docker compose build api`).
- **Test contract:** `src/api/test/project-attachments.e2e.spec.ts` (anon 401 / outsider 403 kể cả project missing / PM upload JPEG 201 + audit → WORKER list/download + `no-store` + `Content-Disposition: attachment` / replay 200 không file mới không audit / fake-exe + >10MB + thiếu file 400 không chạm disk / WORKER retire 403 → PM retire 200 + audit → alreadyInactive lần 2 không audit / history list cờ inactive + download inactive vẫn được / bad correlation 400 / WO extension upload PDF + list + outsider 403 + retire qua WO path — storage THẬT trên tmp dir) + unit policy (mime/size/magic/traversal/caption/reason/requestKey) + unit upload (scope-first, replay-trước-ghi, validate-trước-ghi, orphan cleanup, audit fail 500, race 23505, WO resolve 403/404) + unit read/retire (permissions, scope-bind 404, idempotent, audit fail 500).
