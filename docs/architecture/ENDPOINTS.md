...
- **W4 — optimistic locking:** `PATCH` nhận `expectedConfigVersion` optional; mismatch → `409 {code:'WORK_TYPE_CONFIG_CONFLICT', fieldErrors:{expectedConfigVersion}}` (hai admin cập nhật đồng thời — không mất thay đổi). Không gửi = last-write-wins. Config-relevant field đổi (code/name/group/requiredTradeId/requiredFields) → `config_version` +1 (`versionChanged: true`); chỉ đổi description/duration/priority → không bump. PATCH không thay đổi hiệu lực → no-op, không audit (precedent areas A6).
