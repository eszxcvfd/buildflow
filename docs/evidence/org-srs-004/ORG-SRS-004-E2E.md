...
> **Ngày chạy:** 2026-09-06 (run cuối `e2e-driver-org-srs-004.cjs` — **12/12 PASS** sau fix deep-link B5;
> run trước 11/12 PASS, 1 FAIL product B5 — xem §5e)
> **Phạm vi:** chỉ tạo file mới dưới `docs/evidence/org-srs-004/` — **không commit**; fix source B5
> nằm ở `src/web` (StatusTimeline + AuditLogList + tests), rebuild image web trước run cuối.
...
| --- | --- | --- |
| `admin@example.com` | ADMIN | `E2EAdmin@2025` |
| `pm@example.com` | PROJECT_MANAGER | `E2EPm@2025` (reset từ evidence ORG-SRS-003 §2a) |
...
`docs/evidence/org-srs-004/seed-lifecycle-004.sql` (id prefix **E2E4** để cleanup):

| Bảng | Row | Ghi chú |
| --- | --- | --- |
| `contractors` | `e2e4c000-…c1` — E2E4-CON (ACTIVE, phone `0900123401` **phải hợp lệ** cho validatePhone domain) | nhánh contractor lifecycle |
| `users` | `e2e4a000-…a1` — `e2e4.worker@example.com` (ACTIVE, user_type WORKER) | nhánh worker lifecycle |
| `projects` | `e2e4b000-…b1` E2E4-PRJ ACTIVE | FK chain projects → work_orders → assignments |
| `project_areas` | `e2e4b100-…b2` | `work_orders.area_id` nullable — seed vẫn gắn |
| `work_types` | `e2e4b200-…b3` (required_trade THO-CAT) | `work_orders.work_type_id` NOT NULL |
| `work_orders` | `e2e4b300-…b4` E2E4-WO-1 (status ASSIGNED) | status mở trong CHECK |
| `assignments` | `e2e4c100-…c2` (assignee_type USER, worker E2E4, status PENDING_ACCEPTANCE, requires_acceptance) | **nguồn open-work count = 1** |
| `crews` | `e2e4c200-…c3` (contractor E2E4-CON) | chứng minh nhánh UNION crews.contractor_id |
...
### Cleanup (sau khi chạy xong; audit_logs append-only KHÔNG xóa)
...
| B5 | Timeline 'Lịch sử trạng thái' (WorkerDetail): 3 hành động + lý do + actor; deep-link → audit-logs hiện đúng rows | 🟢 PASS | `B5.png`, `B5-audit.png` | deep-link entityType+entityId+result=SUCCESS trả 16 rows — xem §5 (e) |
...
| B9 | Phân quyền: pm PATCH worker/contractor + GET open-work = **403**; anon = **401**; UI pm card 403 + sidebar ẩn mục Công nhân | 🟢 PASS | `B9-ui.png` | HTTP thật |
...
### (a) Audit trail đầy đủ entity worker E2E4 (12 rows — SUSPEND/REACTIVATE/TERMINATE từ 3 run)
...
------------------------+----------------------------------------------+----------+----------+---------------------------------------+---------+----------
 ...
 ORG_WORKER_SUSPENDED   | Tạm ngừng do thiếu việc trong kỳ (E2E run 3) | ACTIVE   | INACTIVE | Nguồn lực đang có 1 công việc/lịch mở | SUCCESS | f
 ORG_WORKER_TERMINATED  | Chấm dứt hồ sơ do nghỉ việc (E2E run 3)      | ACTIVE   | INACTIVE | Nguồn lực đang có 1 công việc/lịch mở | SUCCESS | f
 ORG_WORKER_REACTIVATED | reset cho B10 (E2E run 3)                    | INACTIVE | ACTIVE   |                                       | SUCCESS | f
 ORG_WORKER_SUSPENDED   | double-submit E2E run 3                      | ACTIVE   | INACTIVE | Nguồn lực đang có 1 công việc/lịch mở | SUCCESS | t
(12 rows)
```
...
### (c) Contractor lifecycle (5 rows — 2 run)
...
----------------------------+------------------------------------------------+----------+----------+----------
 ...
 ORG_CONTRACTOR_SUSPENDED   | Tạm ngừng nhà thầu do chậm tiến độ (E2E run 3) | ACTIVE   | INACTIVE | f
 ORG_CONTRACTOR_REACTIVATED | Hết lý do tạm ngừng (E2E run 3)                | INACTIVE | ACTIVE   | f
(5 rows)
```
...
-- entityType + entityId + result=SUCCESS = 16 rows (1 trang, limit mặc định 20)
...
(`B5-audit.png`, driver assert `seesRows` PASS) — khớp đúng 16 rows DB trên.
...
└── shots/                           (24 ảnh: B0, B1-warning/fielderror, B2-reason/success, B3,
                                      B4-reason/detail, B5 + B5-audit(FAIL), B6-inactive/active,
                                      B7-dialog/detail, B8-detail, B9-ui …)
```
...
- **Dữ liệu E2E còn trong DB**: row prefix `E2E4` (contractor/users/work_orders/assignments/crews)
  + audit append-only (giữ trace). Cleanup §3 nếu cần — audit không xóa (append-only trigger).
- Chỉ có `docs/evidence/org-srs-004/` được thêm; **không commit, không staged**.
