...
const WEB = 'http://localhost:3100'; // REDESIGN rerun: temp next start (fresh .next BUILD_ID 11:26, serves working-tree redesign); compose :3001 image 01:34 = pre-redesign
...
function runCleanup() {
  try {
    ...
    try {
      const prev = JSON.parse(fs.readFileSync(path.join(__dirname, 'e2e-vars-prj001-rerun.json'), 'utf8'));
...
(async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(__dirname, 'e2e-vars-prj001-rerun.json'), JSON.stringify({
