...
| Technical architecture & platform choices | Accepted ADRs ([../architecture/adr/](../architecture/adr/README.md)) | Foundation docs | Old scaffolds and external boilerplates are not architecture authority. |
| Physical data model | DBD V2.1 / Approved CRs / ADRs | SRS V2.1 then BRD V2.0 | DBD may implement a requirement, but cannot invent a business rule. |
...
12. [../architecture/adr/README.md](../architecture/adr/README.md) (Architecture Decision Records)
...
Files outside `docs/foundation/`, `docs/architecture/adr/` and the root `CONTEXT.md` may belong to the old scaffold. They are not product or architecture authority until explicitly reconciled and marked current.
