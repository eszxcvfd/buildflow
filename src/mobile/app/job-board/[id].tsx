...
import { WorkOrderPreviewScreen } from '../../src/features/job-board/WorkOrderPreviewScreen';

export default function JobBoardDetailRoute() {
  ...
  if (!token) {
    return (
      <View style={styles.center}>
        <Text accessibilityRole="alert" style={styles.hint}>Vui lòng đăng nhập để xem chi tiết công việc</Text>
        <Link href="/" accessibilityLabel="go to login" style={styles.link}>Về đăng nhập</Link>
      </View>
    );
  }

  return <WorkOrderPreviewScreen token={token} workOrderId={String(id)} />;
}
